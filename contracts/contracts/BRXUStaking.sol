...
/**
 * @title BRXUStaking
 * @notice Stake $BRXU tokens and earn 12.5% APY rewards.
 ...
 *      Example: 200M reward pool staked → fund 25M BRXU over 365 days.
 *
 * @dev Deployed on Base L2 (Chain ID 8453).
 */
contract BRXUStaking is ReentrancyGuard, Ownable {
    ...
    // ─── Reward State ───────────────────────────────────────────────────
    uint256 public rewardRate;              // BRXU per second
    ...
    constructor(address _brxuToken) Ownable(msg.sender) {
        require(_brxuToken != address(0), "BRXUStaking: zero token");
    ...
    /// @notice Stake BRXU tokens. No lock period.
    function stake(uint256 amount)
        external
        nonReentrant
        updateReward(msg.sender)
    {
        require(amount > 0, "BRXUStaking: zero amount");
    ...
    /// @notice Unstake BRXU tokens. No lock period — withdraw any time.
        ...
        updateReward(msg.sender)
    {
        ...
        require(
            stakedBalances[msg.sender] >= amount,
            "BRXUStaking: insufficient balance"
        );

        stakedBalances[msg.sender] -= amount;
        totalStaked -= amount;

        brxuToken.safeTransfer(msg.sender, amount);
        ...
        updateReward(msg.sender)
    {
        ...
        require(reward > 0, "BRXUStaking: no rewards");

        rewards[msg.sender] = 0;
        brxuToken.safeTransfer(msg.sender, reward);
    ...
    /**
     * @notice Fund the reward pool with BRXU tokens.
    ...
    {
        require(amount > 0, "BRXUStaking: zero amount");
        require(duration > 0, "BRXUStaking: zero duration");
